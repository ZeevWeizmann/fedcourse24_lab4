...
from aggregator import CentralizedAggregator, NoCommunicationAggregator, MedianAggregator
...
def init_client(args, client_id, client_dir, logger):
    ...
    if client_id < num_malicious:
        # malicious client: use Label_Flipping_Learner with same settings
        if args.verbose:
            print(f"[MALICIOUS] client {client_id} uses Label_Flipping_Learner")

        # Build model, criterion, metric, optimizer manually
