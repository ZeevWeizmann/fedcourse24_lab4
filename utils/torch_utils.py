...
def median_models(state_dicts):
    ...
    state_dicts: List[Dict[str, torch.Tensor]]
        List of state_dicts to compute the median from.
    Returns
    -------
    median_state: Dict[str, torch.Tensor]
        State dict containing the coordinate-wise median of the input state_dicts.
    ...
    for k in keys:
        ...
        median_tensor = torch.median(stacked, dim=0).values  # по первой оси
