...
from utils.torch_utils import average_models, copy_model, median_models
...
class Aggregator(ABC):
    ...
    def write_logs(self):
        ...
        # Store last global accuracy
        self.last_test_metric = global_test_metric
    ...
    Returns:
        ...
        if self.sample_with_replacement:
            self.sampled_clients_ids = self.rng.choices(
             population=list(range(self.n_clients)),
             weights=self.clients_weights, k=self.n_clients_per_round)
...
class MedianAggregator(Aggregator):
    ...
    def mix(self):
        ...
        state_dicts = [client.learner.model.state_dict() for client in self.sampled_clients]
